# Language Learning Tool - Feature Roadmap

This document outlines the planned features and improvements for the Language Learning Tool. Each section provides context and requirements that can be used as a reference for implementation.

---

Dictionary Notes
The dictionary serves as the Source of Truth for the users information and usage of words.. Everything goes through and attaches to it. As word are used and have a history and all that, you open up your dictionary and its like opening up your own history book of the learning of the language.

That idea of not just having it be a useful app but also to serve as a historical record keeping of the language learning itself is... interesting. history of the use of the word. essays. buckets. there can even be a little graphic, or eonnected html elements. like

well the word in a box like how it is shown in the bucket section on the dahboard. with a line that links to the next one then thats the second, then the third, etc. Maybe there can be a little crown on the top left corner of the word when its finally marked correct.


### 5. Custom Dictionary Comments
**Status:** TODO
**Priority:** Medium

**Requirements:**
- Allow users to add custom comments to dictionary entries
- Enable tutors to add comments visible to their students
- Create student-specific dictionary feature
- Implement comment visibility controls (private vs. shared)
- community dictionary vs private dictionary

---

### 7. Enhanced Word Details in Bucket Dashboard
**Status:** TODO
**Priority:** Medium

**Requirements:**
- Clicking on a word should show detailed information modal/page
- Display dictionary data for the word
- Show all essays where the word has been used
- Display comments from essays containing the word
- Show context: 30 characters before and after word usage in essays
- Improve essay styling with additional metadata


### 13. Design & UX Improvements
**Status:** TODO
**Priority:** Ongoing

**Requirements:**
- General design improvements across the application
- Enhance user experience throughout
- Address specific UX pain points as they arise

inspiration:
https://www.bergesinstitutespanish.com/home

---

## Not MVP / Future Features

### 14. Multi-Reviewer Essay System
**Status:** In Progress (Phase 2.5/5 Complete - Backend Integration Done)
**Priority:** Medium

**Completed:**
✅ Phase 1: Database Schema (6 migrations)
  - essay_reviews, essay_word_reviews, reviewer_credibility, essay_visibility tables
  - Added consensus tracking to essay_word_join
  - Added multi-reviewer fields to essays table

✅ Phase 2: Backend Models & Services
  - Models: EssayReview, EssayWordReview, ReviewerCredibility, EssayVisibility
  - Services: ConsensusService, CredibilityService, ReviewService
  - Weighted voting algorithm with credibility scoring
  - Auto-approval for high-confidence consensus

✅ Phase 2.5: Backend Integration & API Endpoints
  - EssayService updated to create review sessions automatically
  - TutorController integrated with ReviewService for backward compatibility
  - EssayController updated with visibility management endpoints
  - ReviewController created with all API endpoints:
    * Public review submission
    * Student approval workflow
    * Review summary retrieval
    * Credibility stats and leaderboard
  - Routes configured for public essays and review management

**Next Steps:**
🔄 Phase 3: Student Review Approval Interface (Frontend)
  - ReviewApprovalPage.tsx - Main approval interface
  - ReviewComparisonTable.tsx - Side-by-side word grades
  - ConsensusIndicator.tsx - Visual confidence meter

🔄 Phase 4: Public Essay Sharing (Frontend)
  - PublicEssayView.tsx - Public essay view page
  - EssayVisibilitySettings.tsx - Control public/private
  - AnonymousReviewForm.tsx - Submit review without account

🔄 Phase 5: Credibility System UI (Frontend)
  - ReviewerProfile.tsx - Show stats and tier
  - ReviewerLeaderboard.tsx - Top reviewers
  - CredibilityBadge.tsx - Visual tier indicator

**Requirements:**
- ✅ Create `essay_reviews` table for multiple reviewers
- ✅ Support feedback from tutors, public reviewers, AI
- ✅ Weighted consensus algorithm based on reviewer credibility
- ✅ Student approval system for conflicting reviews (backend)
- ✅ Public essay sharing with shareable links (backend)
- ✅ Anonymous review submission (backend)
- ✅ Reviewer reputation/leaderboard system (backend)
- 🔄 Student review approval UI (frontend)
- 🔄 Public essay view and review form (frontend)
- 🔄 Credibility badges and leaderboard UI (frontend)

**Technical Notes:**
- Uses weighted voting: tutors (0.9), public (0.5), anonymous (0.3)
- Consensus confidence threshold: 60% for student review, 80% for auto-approval
- Credibility formula: (approved_reviews * 0.7) + (helpful_votes * 0.3)
- Backward compatible: Single-tutor essays auto-approve at 100% confidence
- Public essays accessible via `/public/essay/{token}` route

---

### 15. Content Source Tracking
**Status:** Future
**Priority:** Low

**Requirements:**
- Add chapters, articles, or source content when adding words
- Track where vocabulary was encountered
- Link words to original reading material

---

### 16. Essay Sharing & Submission Options
**Status:** Future
**Priority:** Medium

**Requirements:**
- Essay submission modes:
  - Send to nobody (private/draft)
  - Send to tutor immediately
  - Share online publicly
- Implement when dedicated essay pages are complete
- Enable multi-person feedback on single essays

---

### 17. Advanced Grading Features
**Status:** Future
**Priority:** Low

**Requirements:**
- Enable grading of essay portions not from word bank
- Provide feedback on general writing, grammar, structure
- Expand beyond vocabulary-focused grading

 [Lexical vs Tiptap for a WYSIWYG Editor ](https://www.reddit.com/r/reactjs/comments/1929rvs/lexical_vs_tiptap_for_a_wysiwyg_editor/)

Lexical and Tiptap both seem like potential options

---

## Implementation Notes

When implementing any feature from this roadmap:
1. Review the specific requirements listed
2. Examine existing codebase structure and patterns
3. Ensure consistency with current architecture
4. Consider impact on related features
5. Update this document when features are completed or requirements change

### 19. Word History
**Status:** Future
**Priority:** Medium

**Requirements:**
- Can see the history of how the word was used.
- First it was unused.
- Then it was wrong.
- Then it was right.
- Then it was partially correct?
- have access to the essay / tutor / whatever in the history

---

### 3. New Changes / Re-grading after essay submission?
idea: the tutor makes a mistake and needs to fix their mistake

**Status:** TODO
**Priority:** High

**Requirements:**
- Enable tutors to make changes to essays after grading
- Provide ability to re-grade or edit feedback

---

Least Important


### 18. AI-Powered Features
**Status:** Future
**Priority:** Low

**Requirements:**
- AI essay reviewer/feedback system
- Dictionary conjugation tables generated by AI
- AI-generated example sentences for vocabulary
- Automated language assistance features

---

19. Practice Mode vs Exam Mode
- practice mode is just what I've been doing so far. It can have feautres
- like letting users look up words, and so forth and so on while thye are writing
- exam mode: can have custom settings, or have preserts for things like IELTS
- but will prevent any recommended spelling adjustments, will have no feautures
- that give the users more information or hints during writing
- and will potentially have a timer etc

### Study Session
Right now you can go to your bucket to add words. But I think the main crux is
it being "Study Sessions", thats to say, I'm sitting down with my book or my article
and that'st he main source of the activity. So you sit down and start a session
you choose the bucket, you choose what you are doing (LOTR Chapter 1), whatever
and then from there you have the whole what is now "Add Words" feature but its
more of a full-fledged "Study Session" feature.

### small to dos
- move bucket delete functionality to profile page
- should I let tutors have their own buckets and write essay and other normal features of the students? not sure the pros/cons on that.


### User Pain Points - LingQ

<!-- https://www.youtube.com/watch?v=WS3kSezOTvY -->
- "It doesn't actually give you the grammatical aspects of a word if you click on it (shown in sidebar)"
For example (in french) If you click on a word that its in the imperfective case, it won't tell you it's imperfective
, it will only give you the english translation. 

