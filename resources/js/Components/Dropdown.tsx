...
const Content = ({ align = 'right', width = '48', contentClasses = 'py-1 bg-white', children }: PropsWithChildren<ContentProps>) => {
  const context = useContext(DropDownContext);
...
const DropdownLink = ({ className = '', children, ...props }: PropsWithChildren<DropdownLinkProps>) => {
  return (
    <Link
      {...props}
      className={
        'block w-full px-4 py-2 text-start text-sm leading-5 text-neutral-700 transition duration-150 ease-in-out hover:bg-neutral-50 focus:bg-neutral-50 focus:outline-none ' +
        className
      }
    >
      {children}
