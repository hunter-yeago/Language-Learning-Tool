...
export default function TutorEssayViewPage({ essay, student_id }: { essay: Essay; student_id?: number }) {
  ...
  return (
    <AuthenticatedLayout header={<h1 className="text-2xl font-semibold text-gray-800">Essay Review</h1>}>
      ...
      <div className="flex flex-col gap-6 p-6 bg-white shadow-md rounded-lg max-w-4xl mx-auto">
        {/* Back Button */}
        <button
          onClick={() => router.get(route('tutor.students', student_id ? { student_id } : {}))}
        ...
        {/* Student Notes */}
        {essay.notes && (
          <section className="w-full border rounded-lg p-6 bg-blue-50">
            <h3 className="text-lg font-semibold mb-4 text-neutral-900">Student Notes</h3>
            <p className="text-gray-700 whitespace-pre-wrap">{essay.notes}</p>
          </section>
        )}
