...
export default function DictionaryPage() {
  ...
  return (
    <AuthenticatedLayout
      header={
        <h1 className="text-2xl font-semibold text-gray-800">Dictionary</h1>
      }
    >
      ...
      {words.length > 0 && (
        <section
          className="max-w-2xl mx-auto mt-6 p-6 bg-white shadow-md rounded-md"
          aria-label={`dictionary results for ${words[0].entry}`}
        >
          <h2 className="text-xl mb-4 font-bold">
            {capitalizeFirstLetter(words[0].entry)}
          </h2>
            ...
            {words.map((entry) =>
              entry.lexemes.map((lexeme, index) => (
                <LexemeCard
                  key={index}
                  lexeme={lexeme}
                  pluralForm={findPluralForm(words[0].lexemes) ?? ''}
                />
              )),
            )}
          </div>
