...
export default function AuthenticatedLayout({ header, children }) {
    ...
    const renderNavLinks = (isResponsive) => navigationLinks.map(({ href, label }) => (
        isResponsive
            ? <ResponsiveNavLink key={href} href={href} active={route().current(href)}>{label}</ResponsiveNavLink>
            : <NavLink key={href} href={href} active={route().current(href)}>{label}</NavLink>
    ));
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="flex h-16 justify-between">
                        ...
                        <div className="-mr-2 flex items-center sm:hidden">
                            <button onClick={toggleNavigation} className="inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-100 focus:outline-none">
